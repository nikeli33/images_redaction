...
import { FileImage, Download, RefreshCw, ArrowRight } from 'lucide-react';
...
const ConvertMode = () => {
    ...
    const handleFileSelect = (files: FileList | null) => {
        ...
        if (jpgFiles.length === 0) {
            toast.error('Пожалуйста, выберите JPG/JPEG изображения');
            return;
        }

        if (jpgFiles.length !== files.length) {
            toast.warning(`Выбрано только ${jpgFiles.length} JPG изображений из ${files.length}`);
        }

        const newImages: ImageFile[] = jpgFiles.map(file => ({
    ...
    const convertToPng = useCallback(async (imageFile: ImageFile): Promise<ConvertedImage> => {
            ...
            img.onload = () => {
                ...
                canvas.toBlob(
                    (blob) => {
                        ...
                        const name = imageFile.file.name.replace(/\.(jpg|jpeg)$/i, '.png');
    ...
    const handleProcess = useCallback(async () => {
        if (images.length === 0) {
            toast.error('Пожалуйста, выберите JPG изображения');
            return;
        }
        ...
        try {
            const results = await Promise.all(
                images.map((img) => convertToPng(img))
            );

            setProcessedImages(results);
            setShowResults(true);
            toast.success(`Конвертировано ${results.length} изображений в PNG`);
        } catch (error) {
            ...
            <div className="space-y-8">
                <div className="text-center space-y-3">
                    ...
                    <p className="text-muted-foreground">
                        {processedImages.length} изображений конвертировано из JPG в PNG
                    </p>
                </div>
        ...
        <div className="space-y-8">
            <div className="text-center space-y-3">
                ...
                <p className="text-muted-foreground max-w-xl mx-auto">
                    Преобразуйте изображения из формата JPG в PNG с сохранением качества.
                    Обработка происходит прямо в браузере.
                </p>
            </div>
        ...
        `}
            ...
            >
                ...
                <div className="space-y-4">
                    <div className="w-16 h-16 mx-auto bg-gradient-to-br from-orange-500 to-green-500 rounded-2xl flex items-center justify-center">
                        <FileImage className="w-8 h-8 text-white" />
                    </div>

                    <div>
                        <p className="text-lg font-medium text-foreground">
                            Перетащите JPG изображения сюда
                        </p>
                    ...
                    <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span className="px-2 py-1 bg-green-500/10 text-green-600 rounded font-medium">PNG</span>
                    </div>
                </div>
            </div>
